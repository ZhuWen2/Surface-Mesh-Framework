...
//#pragma comment(lib,"D:\\XTOP\\XTOPwork\\mesh\\meshview\\Surface-Mesh-Framework-main\\build\\sanjiaohuadll.lib")
void InteractiveViewerWidget::bilateral() {
	bilateralFilter(mesh);
}

void InteractiveViewerWidget::CloudFusion() {
	Cloud_Reg_Fus();
}

void InteractiveViewerWidget::Triangulation1() {
	TriangulationCloud();
}
...
void InteractiveViewerWidget::mousePressEvent(QMouseEvent *_event)
{
	...
	else
	{
		if(mouse_mode_ != T2_MODE)
		{
			...
			else if (mouse_mode_ == POINT_FILTER)
			{
			}
